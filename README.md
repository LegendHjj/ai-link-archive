...
Firebase web config values are public in any browser-built app, including GitHub Pages. Keep real values in `.env.local` for local development and in your GitHub Pages build secrets/settings for deployment. Security comes from Firebase Authentication and Firestore rules, not from hiding the web config.
